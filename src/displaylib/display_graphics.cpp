/*!
   ...
   @brief Source file for the graphics based functions, 1-bit color displays.
   @author  Gavin Lyons
*/

#include "../../include/displaylib/display_graphics.hpp"
#include "../../include/displaylib/display_fonts.hpp"
#include "../../include/displaylib/ssd1306.hpp"
 ...
 */
displaylib_graphics::displaylib_graphics(int16_t w, int16_t h) : WIDTH(w), HEIGHT(h)
{
	_width = WIDTH;
	_height = HEIGHT;
	_cursor_y = 0;
 ...
 */
DisplayRet::Ret_Codes_e displaylib_graphics::writeChar(int16_t x, int16_t y, char value)
{
	...
	{
		printf("displaylib_graphics::writeChar Error 2: Co-ordinates out of bounds \r\n");
	...
	{
		printf("displaylib_graphics::writeChar Error 3: Character out of Font bounds  %c : %u<->%u \r\n", value, _FontOffset, _FontOffset + _FontNumChars);
 ...
 */
DisplayRet::Ret_Codes_e displaylib_graphics::writeCharString(int16_t x, int16_t y, char *pText)
{
	uint8_t count = 0;
	uint8_t MaxLength = 0;
	...
	if (pText == nullptr)
	{
		print("displaylib_graphics::writeCharString Error 2 :String array is not valid pointer\n");
...
/*!
	@brief Draws a circle with center coordinates (centerX, centerY) and a given radius.
	@param centerX The x-coordinate of the circle's center.
	@param centerY The y-coordinate of the circle's center.
	@param radius The radius of the circle.
	@param color The color of the circle.
*/
void displaylib_graphics::drawCircle(int16_t centerX, int16_t centerY, int16_t radius, uint8_t color)
{
...
/*!
	...
	@param color The color of the circle.
*/
void displaylib_graphics::drawCircleHelper(int16_t centerX, int16_t centerY,
...
*/
void displaylib_graphics::fillCircle(int16_t x0, int16_t y0, int16_t r,
									 uint8_t color)
{
...
*/
void displaylib_graphics::fillCircleHelper(int16_t centerX, int16_t centerY, int16_t radius,
...
*/
void displaylib_graphics::drawLine(int16_t x0, int16_t y0,
								 ...
								 uint8_t color)
{
	...
	if (steep)
	{
		swapInt16display(x0, y0);
		swapInt16display(x1, y1);
	}
	if (x0 > x1)
	{
		swapInt16display(x0, x1);
		swapInt16display(y0, y1);
	}
	int16_t dx, dy;
	dx = x1 - x0;
	dy = abs(y1 - y0);
...
*/
void displaylib_graphics::drawRect(int16_t x, int16_t y,
								   int16_t w, int16_t h,
								   uint8_t color)
{
...
*/
void displaylib_graphics::drawFastVLine(int16_t x, int16_t y,
...
/*!
	...
	@param color The color of the line
*/
void displaylib_graphics::drawFastHLine(int16_t x, int16_t y,
...
*/
void displaylib_graphics::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
								   uint8_t color)
{
...
*/
void displaylib_graphics::drawRoundRect(int16_t x, int16_t y, int16_t w,
...
*/
void displaylib_graphics::fillRoundRect(int16_t x, int16_t y, int16_t w,
										int16_t h, int16_t r, uint8_t color)
{
...
*/
void displaylib_graphics::drawTriangle(int16_t x0, int16_t y0,
									   int16_t x1, int16_t y1,
...
*/
void displaylib_graphics::fillTriangle(int16_t startX0, int16_t startY0,
									   ...
									   int16_t startX2, int16_t startY2, uint8_t color)
{
	...
	if (startY0 > startY1)
	{
		swapInt16display(startY0, startY1);
		swapInt16display(startX0, startX1);
	}
	if (startY1 > startY2)
	{
		swapInt16display(startY2, startY1);
		swapInt16display(startX2, startX1);
	}
	if (startY0 > startY1)
	{
		swapInt16display(startY0, startY1);
...
*/
void displaylib_graphics::setCursor(int16_t x, int16_t y)
{
	_cursor_x = x;
	_cursor_y = y;
}
...
*/
DisplayRet::Ret_Codes_e displaylib_graphics::drawBitmap(int16_t x, int16_t y,
...
// === ADVANCED Graphics class implementation ===
 ...
 */
DisplayRet::Ret_Codes_e displaylib_graphics::drawPolygon(int16_t x, int16_t y, uint8_t sides, int16_t diameter, float rotation, bool fill , uint8_t color) 
{
	if ((sides < 3 ) || (sides > 10)) {
		printf("Error : drawPolygon : sides incorrect value 3-10\r\n");
		return DisplayRet::GenericError;
	}
...
/*!
	...
	@return DisplayRet::Ret_Codes_eReturns a status code indicating success or failure. Possible return values:
	- DisplayRet::Success: The grid of dots was successfully drawn.
	- DisplayRet::ShapeScreenBounds: The provided coordinates or dimensions are out of screen bounds.
 */
DisplayRet::Ret_Codes_e displaylib_graphics::drawDotGrid(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t DotGridGap, uint8_t color)
{
	//User input handling
	if ((x >= _width) || (y >= _height)){
		printf("drawDotGrid: Out of screen bounds\n");
		return DisplayRet::ShapeScreenBounds;
	}
...
/*!
	...
	@param color The color of the line
 */
void displaylib_graphics::drawLineAngle(int16_t x, int16_t y, int angle, uint8_t start, uint8_t length, int offset , uint8_t color)
{
	if (start == 0){
		if (length < 2) {
			drawPixel(x, y, color);
 ...
 */
void displaylib_graphics::drawQuadrilateral(int16_t x0, int16_t y0,int16_t x1, int16_t y1,int16_t x2, int16_t y2,int16_t x3, int16_t y3, uint8_t color) 
{
 ...
 */
void displaylib_graphics::fillQuadrilateral(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t x3, int16_t y3, uint8_t color)
{
 ...
 */
void displaylib_graphics::drawEllipse(int16_t cx, int16_t cy, int16_t semiMajorAxis, int16_t semiMinorAxis,  bool fill, uint8_t color)
{
	int16_t x, y; 
 ...
 */
void displaylib_graphics::ellipseHelper(uint16_t cx, uint16_t cy, uint16_t x, uint16_t y, uint8_t color)
{
...
*/
void displaylib_graphics::drawArcHelper(uint16_t centerX, uint16_t centerY, uint16_t radius, uint16_t thickness, float start, float end, uint8_t color) 
{
...
/*!
	...
	@note variables _arcAngleMax and  _arcAngleOffset which can be set by user with setters
		...
		XXX 090 XXX
 */
void displaylib_graphics::drawArc(uint16_t cx, uint16_t cy, uint16_t radius, uint16_t thickness, float startAngle, float endAngle, uint8_t color) {
 ...
 */
void displaylib_graphics::drawSimpleArc(int16_t cx, int16_t cy, int16_t radius, float startAngle, float endAngle, uint8_t color)
{
	const float degreesToRadians = std::numbers::pi / 180.0;
